 ...
 *
 * Do   " cl /Fe:mcxyz mcxyz.c "
 * then " mcxyz skinvessel "
 * 
 *  USAGE   mcxyz myname
...
int main(int argc, const char * argv[]) {
	...
	if (mcflag==4) printf("launching isotropic square source\n");
	...
	// Create angle matrix (count, cumulative phi, cumulative theta)
	float 	*Ang=NULL;
	int saves = 3;
	Ang  = (float *)malloc(Nx*Ny*saves*sizeof(float));
	for(j=0; j<Nx*Ny*saves;j++) 	Ang[j] = 0;
	...
	do {
		...
		if (i_photon==1000) {    
			finish_time = clock();
			time_min = (long)Nphotons*(finish_time-temp_time)/(60*999*CLOCKS_PER_SEC);
			printf("Simulation time = %0.2f min for Nphotons = %0.0f \n",time_min, Nphotons);
		}
		...
		if (launchflag==1) { // manually set launch
			...
			if (mcflag==0) { // uniform beam
				...
				costheta = 1.0*RandomGen(1,0,NULL);
			...
			} else if (mcflag==4) { // rectangular isotropic source 
				while ((rnd = RandomGen(1,0,NULL)) <= 0.0); // avoids rnd = 0
				x = radius*(rnd*2-1); // use radius to specify x-halfwidth of rectangle
				while ((rnd = RandomGen(1,0,NULL)) <= 0.0); // avoids rnd = 0
				y = radius*(rnd*2-1); // use radius to specify y-halfwidth of rectangle
		...
		/* Save theta and psi angles of photon direction at termination,
		in voxel position it was terminated in. */
		i = (long)((ix*Ny + iy)*saves);
		Ang[i + 0] += 1;
		double temp = 0.0;
		if (ux == 0) {
			if (uy > 0) temp = PI/2.0;
			if (uy < 0) temp = -PI/2.0;
			else printf("ux, uy are both 0, Phi is undefined");
		} else if (ux > 0) {
			temp = atan(uy/ux);
		} else if (ux < 0) {
			if (uy >= 0) temp = atan(uy/ux) + PI;
			if (uy < 0) temp = atan(uy/ux) - PI;
		}
		Ang[i + 1] += temp;
		Ang[i + 2] += acos(uz);	
	...
	strcpy(filename,myname);
	strcat(filename,"_Ang.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 3D voxel output */
    fwrite(Ang, sizeof(float), Nx*Ny*saves, fid);
    fclose(fid);
